...
const dummyRequestData: RequestData[] = [
  { date: "15-02-2025", title: "Equipment repair approval request", lastUpdated: "20-02-2025" },
  ...
  { date: "20-01-2025", title: "Conference travel fund request", lastUpdated: "28-01-2025" },
  ...
  { date: "22-11-2024", title: "Building maintenance request", lastUpdated: "30-11-2024" },
...
const Request = () => {
  ...
  const stats = useMemo(() => {
    return dummyRequestData.reduce(
      (acc) => {
        acc.total += 1;
        return acc;
      },
  ...
  const [initialData, setInitialData] = useState<RequestData[]>([])
  const [filteredData, setFilteredData] = useState(dummyRequestData)


  // Functions
  useEffect(() => {
    const filterData = (RequestData: RequestData[], filterKey: keyof RequestData | undefined, filterValue: string | undefined, page: number) => {

      let tmpData = [...RequestData];
  ...
  useEffect(() => {
    if (!initialData || initialData.length === 0) {
      setInitialData(dummyRequestData)
    }
  }, [initialData])
  ...
  return (
    <div className={`${styles.request_container} ${styles[theme]}`}>
      <div className={styles.title}>Request Model</div>
